...
void RTObj::init(const char * filename){
    ...
    // setting up triangle elements
    std::cout << "Setting up triangles...";
    for(int i=0; i<n; i+=3){
        Triangle nextTri;
        nextTri.P.push_back(glm::vec3(vertices[indices[i]][0], vertices[indices[i]][1], vertices[indices[i]][2]));
        nextTri.P.push_back(glm::vec3(vertices[indices[i+1]][0], vertices[indices[i+1]][1], vertices[indices[i+1]][2]));
        nextTri.P.push_back(glm::vec3(vertices[indices[i+2]][0], vertices[indices[i+2]][1], vertices[indices[i+2]][2]));
        nextTri.N.push_back(glm::vec3(normals[indices[i]][0], normals[indices[i]][1], normals[indices[i]][2]));
        nextTri.N.push_back(glm::vec3(normals[indices[i+1]][0], normals[indices[i+1]][1], normals[indices[i+1]][2]));
        nextTri.N.push_back(glm::vec3(normals[indices[i+2]][0], normals[indices[i+2]][1], normals[indices[i+2]][2]));
        elements.push_back(nextTri);
    }
    count = n;
